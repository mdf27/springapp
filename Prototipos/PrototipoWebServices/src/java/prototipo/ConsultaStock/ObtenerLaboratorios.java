...
public class ObtenerLaboratorios extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
           ...
           // URL rutaService= new URL ("http://dev.dusa.com.uy/ws_consulta_stock?wsdl");
            try {
                ...
                if (laboratorios.isEmpty()) {
                    out.println("<h1>Lista de Laboratorios es vacia </h1>");
                } else {
        ...
        "                var xmlhttp=new XMLHttpRequest();\n" +
        "                xmlhttp.open(\"POST\",\"crearPdf\",true);// va una ruta ahi pa q atiendan la request\n" +
        "                xmlhttp.setRequestHeader(\"Content-type\",\"application/x-www-form-urlencoded\");\n" +
        "                var parametro='id='+id; \n" +
        "                xmlhttp.send(parametro); \n" +
        "            }\n" +
        "        </script> "); 
                    ...
                    out.println("<p>(Hacer click en una fila para ver detalles del laboratorio)</p>");
                    ...
                    while(it.hasNext()) {
                        ...
                        "<td>"+ razonSocial +"</td>\n" +
                        "<td>"+ ruc +"</td>\n" +  
                        "<td><a class=\"button\" onclick=\"generarPDF('"+ idLab + "')\">Imprimir</a></td>\n" +          
                        "</tr>\n"); 
                    }
                    out.println("</tbody>\n" +
                    "</table>");
